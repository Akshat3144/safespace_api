...
import { insertPropertySchema, insertNeighborhoodSchema, insertCompareListSchema, } from "../shared/schema.js"; // Ensure the file exists and the extension matches your setup
import { z } from "zod";
export async function registerRoutes(app) {
    ...
    app.get("/api/properties", async (req, res) => {
        try {
            ...
            if (minHdi) {
                properties = properties.filter((p) => p.hdiScore != null && p.hdiScore >= Number(minHdi));
            }
            res.json(properties);
        }
        catch (error) {
